...
export default function GalleryGrid({ items, onSelect }: GalleryGridProps) {
  ...
  const sourceItems = items && items.length ? items : fetchedItems;
      ...
      (entries) => {
        ...
        if (entry.isIntersecting) {
          setVisibleCount((current) =>
            Math.min(current + ITEMS_PER_PAGE, sourceItems.length)
          );
        }
      },
      {
  ...
  }, [hasMore, sourceItems.length]);

  // Fetch from Supabase when `items` prop is not provided
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (error) {
          ...
          setFetchedItems((data as GalleryItem[]) || []);
        }
      } catch (err: any) {
        if (!mounted) return;
  ...
  if (fetchError) {
    return (
      <div className="text-sm text-red-400">Gagal memuat gallery: {fetchError}</div>
    );
  }
